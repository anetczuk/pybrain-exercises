...
from pybrain.rl.environments.ode.tasks import StandingTask, RStandingTask, RobStandingTask, RobustStandingTask, JumpingTask, StandingUpTask
...
TASK_MAP = { "stand1":  StandingTask,
             "stand2":  RStandingTask, 
             "stand3":  RobStandingTask, 
             "stand4":  RobustStandingTask, 
             "jump":    JumpingTask, 
             "standup": StandingUpTask
            }
...
def save_data( data, output_file ):
    outPath = output_file
    if outPath is None:
        outPath = os.path.join( TMP_DIR, "johnnie_pgpe.txt" )
#     print '\nstoring params:', data
...
def create_environment( use_renderer, learn, load_data, inner_layers, bias_unit, batch, alpha, task_type=None ):
    ## create task
    env = JohnnieEnvironment( renderer=use_renderer )
    if learn:
        env.render = False
    else:
...
#     env.dt = 0.019
    ...
    if task is None:
        ...
        if task_class is None:
            task_class = StandingTask
        print "using task:", task_class
        task = task_class(env)
    else:
        task.env = env
...
#                 learner.sigmaLearningRate = alpha / 2.0
...
def main( seed=None, steps=200, batch=5, bias_unit=True, inner_layers=[], alpha=0.1, use_renderer=False, learn=True, task_type=None,
    ...
    env, task, net, learner, agent, experiment = create_environment( use_renderer, learn, load_data, inner_layers, bias_unit, batch, alpha, task_type )
...
if __name__ == '__main__':
    ...
    parser.add_argument( '-b',   '--batch', action="store", type=int, default=2, help='Batch' )
    ...
    parser.add_argument(         '--task', type=str, default=None, help='Task, one of: ' + str(taskValues) )
    ...
    main( seed=args.seed, steps=args.steps, batch=args.batch,
          ...
          use_renderer=args.renderer, learn=args.learn, task_type=args.task,
